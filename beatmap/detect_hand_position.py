...
def detect_hand_position_draw(frame, interpreter, hands, mp_hands, mp_drawing, input_details, output_details, use_double=True):
    #Unmirror the frame
    ...
    if results.multi_hand_landmarks and results.multi_handedness:
        for idx, (hand_landmarks, handedness) in enumerate(zip(results.multi_hand_landmarks, results.multi_handedness)):
            ...
            positions = detect_hand_position(frame, interpreter, hands, mp_hands, input_details, output_details, use_double=True)
            # Left hand position, then right hand position

        if use_double:
            # Get hand label (Left or Right)
            text = f"{DOUBLE_ID_TO_GESTURE[positions[0]]}"
...
def detect_hand_position(frame, interpreter, hands, mp_hands, input_details, output_details, use_double=True):
    """
    ...
    frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    results = hands.process(frame_rgb)
    positions = [len(DOUBLE_LABEL_MAP)-1, len(DOUBLE_LABEL_MAP)-1]
...
def test_hand_position_live(cap, hands, mp_hands, mp_drawing, interpreter, input_details, output_details, use_double=True):
    while cap.isOpened():
        ret, frame = cap.read()
        if not ret:
            break

        frame, positions = detect_hand_position_draw(frame, interpreter, hands, mp_hands, mp_drawing, input_details, output_details, use_double=True)
        # frame, positions = detect_hand_position_draw(frame, hands, mp_hands, mp_drawing, input_details, output_details)
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
##LANDMARK
def check_hand_position(octet_stream, hands, mp_hands, interpreter, input_details, output_details, use_double = True):
    """
    ...
    cap.release()
    gesture_counts_left = [0] * (len(SINGLE_LABEL_MAP) + 1) if not use_double else [0] * (len(DOUBLE_LABEL_MAP) + 1)
    gesture_counts_right = [0] * (len(SINGLE_LABEL_MAP) + 1) if not use_double else [0] * (len(DOUBLE_LABEL_MAP) + 1)
...
def check_hand_position_api(bin_file, mp_hands, hands, interpreter, input_details, output_details, use_double):
    with open(bin_file, "rb") as f:
        octet_stream = f.read()
    left, right = check_hand_position(octet_stream, hands, mp_hands, interpreter, input_details, output_details, use_double = True)
    print(left)
    return left, right

def main(use_double = True):
    print(DOUBLE_GESTURES, DOUBLE_ID_TO_GESTURE)
    ...
    # Test hand position
    test_hand_position_live(cap, hands, mp_hands, mp_drawing, interpreter, input_details, output_details, use_double = use_double)
